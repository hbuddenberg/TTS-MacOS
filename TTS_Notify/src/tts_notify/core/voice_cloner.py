...
from .models import (
    Voice, VoiceCloningRequest, VoiceCloningResponse, VoiceProfile,
    Gender, Language, VoiceQuality, TTSEngineType
)
...
class VoiceCloner:
    ...
    def _setup_directories(self):
        ...
        # Profile directory
        if self.config.TTS_NOTIFY_COQUI_PROFILE_DIR:
            self.profile_dir = Path(self.config.TTS_NOTIFY_COQUI_PROFILE_DIR)
        else:
            self.profile_dir = Path.home() / ".tts-notify" / "voice-profiles"

        # Embedding directory
        if self.config.TTS_NOTIFY_COQUI_EMBEDDING_DIR:
            self.embedding_dir = Path(self.config.TTS_NOTIFY_COQUI_EMBEDDING_DIR)
        else:
            self.embedding_dir = Path.home() / ".tts-notify" / "embeddings"

        # Temp directory
        self._temp_dir = Path.home() / ".tts-notify" / "temp" / "cloning"

        # Create directories
        for dir_path in [self.profile_dir, self.embedding_dir, self._temp_dir]:
    ...
    async def clone_voice(self, request: VoiceCloningRequest) -> VoiceCloningResponse:
        ...
        try:
            logger.info(f"Starting voice cloning: {request.voice_name} ({request.language})")
            ...
            # Create voice object
            voice = await self._create_cloned_voice(request, voice_profile, sample_duration)
            ...
            logger.info(f"Voice cloning completed successfully in {processing_time:.2f}s")
    ...
    async def _process_source_audio(self, request: VoiceCloningRequest) -> Tuple[np.ndarray, float]:
        """
        ...
        try:
            ...
            if duration < min_duration:
                raise ValidationError(f"Audio too short: {duration:.2f}s (minimum: {min_duration}s)")
            if duration > max_duration:
                raise ValidationError(f"Audio too long: {duration:.2f}s (maximum: {max_duration}s)")
            ...
            if quality_config["sample_rate"] != sr:
                audio = librosa.resample(y=audio, orig_sr=sr, target_sr=quality_config["sample_rate"])
    ...
    async def _generate_voice_embedding(self, audio: np.ndarray, voice_name: str, language: str) -> Path:
        """
        ...
        try:
            ...
            embedding_filename = f"{voice_name}_{language}_{timestamp}.npy"
    ...
    async def _create_voice_profile(self, request: VoiceCloningRequest, embedding_path: Path,
                                   sample_duration: float) -> VoiceProfile:
        ...
        try:
            ...
            # Generate metadata
            metadata = {
                "source_file": str(request.source_audio_path),
    ...
    async def _optimize_voice_profile(self, profile: VoiceProfile) -> float:
        ...
        try:
            ...
            if not profile.embedding_path.exists():
                raise ValidationError(f"Embedding file not found: {profile.embedding_path}")
    ...
    async def _create_cloned_voice(self, request: VoiceCloningRequest, profile: VoiceProfile,
                                 sample_duration: float) -> Voice:
        """Create Voice object from profile"""
        try:
            # Map language to Language enum
            language_map = {
                "en": Language.ENGLISH, "es": Language.SPANISH, "fr": Language.FRENCH,
                "de": Language.GERMAN, "it": Language.ITALIAN, "pt": Language.PORTUGUESE,
                "nl": Language.UNKNOWN, "pl": Language.UNKNOWN, "ru": Language.UNKNOWN,
                "zh": Language.CHINESE, "ja": Language.JAPANESE, "ko": Language.KOREAN
            }
    ...
    def _denoise_audio(self, audio: np.ndarray) -> np.ndarray:
        ...
        try:
            # Simple spectral denoising (simplified)
            # Real implementation would use advanced denoising algorithms

            # Apply high-pass filter to remove low-frequency noise
            from scipy import signal
            sos = signal.butter(10, 80, btype='high', fs=self.processing_config.sample_rate, output='sos')
    ...
    async def get_cloning_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            profile_size = sum(f.stat().st_size for f in self.profile_dir.glob("*.json") if f.is_file())
            embedding_size = sum(f.stat().st_size for f in self.embedding_dir.glob("*.npy") if f.is_file())
