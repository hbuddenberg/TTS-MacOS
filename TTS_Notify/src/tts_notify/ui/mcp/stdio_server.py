#!/usr/bin/env python3
"""
TTS Notify v2.0.0 - Stdio MCP Server

Manual MCP server implementation that works with stdio communication.
...
class TTSNotifyMCPServer:
    ...
    async def speak_text(self, text: str, voice: Optional[str] = None, rate: Optional[int] = None) -> str:
    ...
    async def save_audio(self, text: str, output_path: str, voice: Optional[str] = None, rate: Optional[int] = None) -> str:
        ...
        try:
            if method == "initialize":
                ...
                if tool_name == "speak_text":
                    ...
                    result = await self.list_voices()
                elif tool_name == "save_audio":
                    result = await self.save_audio(**arguments)
                else:
